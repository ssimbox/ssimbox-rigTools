...
def jointMaker(*args):
    ...
    print(gameExpGrp)
    if cmds.objExists(gameExpGrp + "1"):
        cmds.delete(gameExpGrp)
    ...
    for i in gameJoints:
        ...
        # if cmds.objExists(gameExpGrp):


    #Parent constraint from rig_jnts to game_jnts
def makeConstraint(*args):
    for i in range(len(mainJoints)):
        #print (mainJoints[i], gameJoints[i])
        # if cmds.objExists(i+'_game'):
        cmds.parentConstraint(mainJoints[i], gameJoints[i], mo=1)
...
def showUI():
    ...
    suffix_UI = cmds.textField(w=100, tx="_bind")
